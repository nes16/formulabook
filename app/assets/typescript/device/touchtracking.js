...
define(['require', 'jquery', 'device/ipadghostevents'], function (require) {
    ...
    var dispatchEvent = function (type, evnt, differentTarget) {
        ...
        if (type === 'tlab-tapstart') {
            ...
            if (mode === TOUCH_MODE && lastEvnt && lastEvnt.type === 'tlab-tapstart' &&
    ...
    $(document).on('pointerdown MSPointerDown', function (evnt) {
        if (mode === MOUSE_MODE || !isMSTouchEvent(evnt))
            return;
        // setup TOUCH_MODE
        if (mode === NO_MODE) {
            beginMode(evnt);
    ...
    $(document).on('pointermove MSPointerMove', function (evnt) {
        if (mode !== TOUCH_MODE || !isMSTouchEvent(evnt))
            return;
    ...
    $(document).ready(function () {
        $(document).on("mousedown", function (e) {
            var doNotBlur = !!$(e.target).closest('.tlab-do-not-blur').length, doBlur = !!$(e.target).closest('.tlab-do-blur').length;
            if (doNotBlur && !doBlur) {
                e.preventDefault();
            }
