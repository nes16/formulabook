    ...
    var TestLabAPI = P(function (proto) {
        function validatedOptions(options) {
            ...
            if (options.hasOwnProperty('menus')) {
                console.warn('As of API version 0.4, the \'menus\' option is deprecated and has been split into ' +
                    'settingsMenu (boolean) and expressionsTopbar (boolean).');
                if (!options.hasOwnProperty('settingsMenu'))
                    out.settingsMenu = !!options.menus;
                if (!options.hasOwnProperty('expressionsTopbar'))
                    out.expressionsTopbar = !!options.menus;
            }
            if (!out.graphpaper) {
                if (out.expressionsCollapsed) {
                    out.expressionsCollapsed = false;
                    console.warn('Desmos API initialized with bad options. graphpaper: false ' +
